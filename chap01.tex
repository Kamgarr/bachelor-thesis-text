...
\para{Material point method} (MPM) is used to simulate the behaviour of continuum materials, continuum material is represented as continuous mass not a set of discrete particles. MPM is a particle method and uses material points (Lagrangian elements) to describe a body. Each material point stores and calculates its own position, velocity and deformation gradient. The difference between MPM and FEM is that the MPM is a meshfree method. Meshfree method stores all its information in the particles and avoids a numerical errors from remeshing algorithms. In the context of numerical method MPM is a Arbitrary Lagrangian–Eulerian numerical method \cite{ALE}, this means that calculations do not only use material points, but also a Eulerian grid. A simplified overview of algorithm steps follows, for more details see the thesis of \citet{jiang2015material}.
...
In the soft body, application of the force propagates across the particles and the connected particle break apart when the limit is exceeded. This creates the fracturing that may lead to the whole body splitting. In the rigid body simulation, the body has no internal structure and therefore the applied force can only change the momentum of the whole body. This prohibits the deformation or fracturing a rigid body in simulation.

To handle this problem, there are numerous approaches of decomposing the rigid body in the way that imitates fractures achieved by exceeding the elasticity of the material. The most common approach is a decomposition of rigid body into smaller parts. A few of the methods for decomposition are slicing by planes, convex decomposition, tetrahedralization and Voronoi tesselation. 
...
\begin{description}
\item[Destruction on a coarse scale] on this level the authors use Extended Distinct Element Method (EDEM) which is based on Element Method described in section \ref{sec:softBody}. Instead of using particles, EDEM uses a set of rigid bodies that need to be produces by some tessellation method. The extension over traditional DEM is the use of pore springs connecting the adjacent elements. The authors compare the elements to individual bricks (the distinct elements) held together by layers of mortar (the pore springs). If the applied force is sufficient and elements move apart far enough, the fracture takes place. Pore springs also help the object retain its original shape after applied force.
\begin{figure}[ht!]
        \centering
...
\todo{proc teda publikovali game approach kterej je ve hrach nepouzitelnej? V tom clanku asi bude nejakej lepsi conclusion.}
